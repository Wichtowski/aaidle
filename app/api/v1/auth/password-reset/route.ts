...
import { consumeRateLimit, createPasswordResetToken } from "@/lib/auth/auth-service";
...
export async function POST(request: Request) {
  try {
    ...
    if (code === "EMAIL_DELIVERY_NOT_CONFIGURED" || code === "EMAIL_DELIVERY_FAILED") {
      return authError(code, "We could not send the reset email. Try again later.", 503);
    }
