...
export function createTrajectoryAccessToken({ challengeId, answerModelId }: TrajectoryAccessPayload) {
...
export function hasTrajectoryAccess(
  ...
  try {
    const parsed = JSON.parse(Buffer.from(payload, "base64url").toString("utf8")) as Partial<TrajectoryAccessPayload>;
    return parsed.challengeId === expected.challengeId && parsed.answerModelId === expected.answerModelId;
  } catch {
    return false;
  }
}
