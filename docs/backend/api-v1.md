...
Emoji image clues may reference `/emoji-visual/`, `/common/`, or shared `/logo-visual/` public files.
...
`GET /api/v1/games/logo/normal` returns the daily Logo challenge, its independent catalog answer pool, and server-authoritative reveal progress. The safe progress object contains a player-scoped API image URL, profile-specific reveal settings, the current and maximum image revisions, unlocked clues, and solved state.
Logo progress in game, guess, and history responses includes `revealProfile`. A `null` profile serves the original image bytes without resizing, cropping, blurring, or other image processing. `"progressive-zoom"` includes `focalPoint` (`x` and `y` in 0–512). `"gaussian-blur"` includes `blurStartStrength` and `blurStepStrength` instead and omits `focalPoint`; both strengths must be finite numbers greater than 0 and at most 64. The API blurs the full image, resized to fit 512×512 while preserving its aspect ratio. The Gaussian sigma is `max(blurStartStrength - imageRevision * blurStepStrength, 0)`. Revision is capped at seven for both profiles, and server-confirmed completion always returns an unblurred image regardless of the remaining strength. Unlocked auxiliary image clues are served unblurred. Rendered cache keys include the profile and its parameters; the loaded original remains shared across profiles.
`GET /api/v1/games/logo/challenges/{challengeId}/image?token={capability}` resolves the anonymous or authenticated player identity and serves the authorized server-rendered image.
The HMAC-signed capability is random, expires at the next UTC midnight, and is bound to the challenge, resolved player, and exact main or clue variant.
Changing its payload or signature, using it under another player session, or using it after expiry returns `404`.
The endpoint also compares the signed variant with current persisted progress, so an otherwise valid capability cannot unlock a stage the player has not reached.
Source images are private files below `LOGO_ASSET_DIR` and are bundled only with the backend.
Only root-relative PNG/WebP identifiers are accepted, and canonical path checks prevent reads outside the configured directory.
Files are capped at 10 MiB; decoding is bounded to 8192 pixels per dimension and a 128 MiB allocation limit.
Unavailable, oversized, or invalid image sources return `503` without disclosing the source path.
Failed reads and decodes are not cached and can be retried after repairing the source.
Originals have a 24-hour in-memory TTL; expiry also discards their rendered variants.
Changing the active challenge or restarting the API clears both caches.
Responses use `Cache-Control: private, no-store`.
`GET /api/v1/games/logo/challenges/{challengeId}/guesses` restores accepted guesses and reveal progress.
`POST` to the same route accepts retry-safe `requestId`, `guessedModelId`, and sequential `attemptNumber` fields.
Every incorrect accepted guess advances the bounded image revision.
Clues remain absent until their server-configured `afterIncorrectGuesses` threshold; `0` makes a clue available in the initial game and history responses.
Each clue contains `afterIncorrectGuesses`, `kind`, and `text`.
Image clues also contain a capability-bearing `imageUrl`; source asset identifiers are not included in Logo responses.
The image endpoint returns 404 for locked, nonexistent, non-image, malformed, expired, or wrong-player capabilities and expired challenges.
Catalog entries may use educational, general, or image clues.
The browser records opened clues locally for the current player and challenge, marks them viewed, and counts each once in completion/share summaries; this presentation state never grants access to clues.
A correct guess never serializes the hidden answer field.
...
`POST /api/v1/issues` requires a signed-in, non-disabled account and a same-origin request. It accepts a `game` of `classic`, `emoji`, `timeline`, or `logo`, a `title` from 8 to 120 characters, and a `description` from 20 to 5,000 characters, then creates an issue in the project tracker with the GitHub labels `user-report`, `needs-triage`, and the selected game using the server-side `GITHUB_ISSUES_TOKEN`. These server-assigned labels identify reports submitted through the app and awaiting initial review; clients do not choose triage labels. Each account may submit two reports per rolling 24-hour period by default; a superadmin may set an account's limit from 0 (reporting disabled) to 1,000. The endpoint returns the created issue URL. The GitHub token is never sent to the browser or logged.
