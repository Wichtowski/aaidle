...
Logo progress in game, guess, and history responses includes `revealProfile`. A `null` profile serves the original image bytes without resizing, cropping, blurring, or other image processing. `"progressive-zoom"` includes `focalPoint` (`x` and `y` in 0–512). `"gaussian-blur"` includes `blurStartStrength` and `blurStepStrength` instead and omits `focalPoint`; both strengths must be finite numbers greater than 0 and at most 64. The API blurs the full image, resized to fit 512×512 while preserving its aspect ratio. The Gaussian sigma is `max(blurStartStrength - imageRevision * blurStepStrength, 0)`. Revision is capped at seven for both profiles, and server-confirmed completion always returns an unblurred image regardless of the remaining strength. Unlocked auxiliary image clues are served unblurred. Rendered cache keys include the profile and its parameters; downloading the original remains shared across profiles.
`GET /api/v1/games/logo/challenges/{challengeId}/image?v={variant}` resolves the anonymous or authenticated player identity and serves the authorized server-rendered crop. For the main image, a requested variant that does not equal the caller's persisted revision returns `404`; only `v=solved` after a server-confirmed solve returns the full main image. Unlocked image clues use the `clue-{index}` variants described below. Source images are public static files under `public/`, while catalog JSON remains backend-only. The API resolves the seed's root-relative `assetUrl` against `APP_ORIGIN` and downloads it without forwarding player cookies or following redirects. Only PNG/WebP source paths are accepted. Downloads are capped at 10 MiB and use `REQUEST_TIMEOUT_SECONDS`; decoding is bounded to 8192 pixels per dimension and a 128 MiB allocation limit. Unavailable, oversized, timed-out, redirected, or invalid image sources return `503` without disclosing the source URL. Failed downloads/decodes are not cached and can be retried after repairing the source. Originals have a 24-hour in-memory TTL; expiry also discards their rendered variants. Changing the active challenge or restarting the API clears both caches. Responses use private browser caching bounded by the next UTC challenge.
`GET /api/v1/games/logo/challenges/{challengeId}/guesses` restores accepted guesses and reveal progress. `POST` to the same route accepts retry-safe `requestId`, `guessedModelId`, and sequential `attemptNumber` fields. Every incorrect accepted guess advances the bounded image revision. Clues remain absent until their server-configured `afterIncorrectGuesses` threshold (a nonnegative integer); `0` makes a clue available in the initial game and history responses. Each clue contains `afterIncorrectGuesses`, `kind`, and `text`. Image clues also contain `imageUrl`, pointing to the protected image endpoint with `v=clue-{index}` (zero-based index in the ordered clues array); source asset URLs are not included in Logo challenge/guess responses. The image endpoint returns 404 for locked, nonexistent, non-image, or malformed clue variants and expired challenges. Catalog entries may use educational, general, or image clues. The browser records opened clues locally for the current player and challenge, marks them viewed, and counts each once in completion/share summaries; this presentation state never grants access to clues. A correct guess never serializes the hidden answer field.
