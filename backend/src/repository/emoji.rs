...
pub async fn guess_history(
    ...
    challenge_id: Uuid,
    player_id: Uuid,
) -> AppResult<Vec<EmojiDifficultyGuessHistoryEntry>> {
    if challenge_by_id(pool, challenge_id).await?.is_none() {
        return Err(AppError::NotFound("Emoji challenge not found.".to_owned()));
    }
    ...
    for stored in stored {
        let entity = catalog.entity(&stored.guessed_entity_id).ok_or_else(|| {
            AppError::Unavailable("Stored Emoji guess is unavailable.".to_owned())
        })?;
        guesses.push(EmojiDifficultyGuessHistoryEntry {
            id: entity.id.clone(),
...
async fn process_guess_once(
    ...
    let entity = catalog
        ...
        .cloned()
        .ok_or_else(|| AppError::validation("This answer is not in the Emoji pool."))?;
    let difficulty = challenge
        .mode
        .strip_prefix(MODE_PREFIX)
        .and_then(difficulty_pool)
        .ok_or_else(|| AppError::Unavailable("Emoji challenge has an invalid mode.".to_owned()))?;
    if entity.min_pool > difficulty {
        return Err(AppError::validation(
            "This answer is not in the Emoji pool.",
        ));
    }
    let pool_size = i64::try_from(catalog.eligible(difficulty).count())
        .map_err(|_| AppError::Unavailable("Emoji attempt limit is invalid.".to_owned()))?;
...
async fn ensure_challenge(
    ...
    let rank = difficulty_pool(difficulty)
        .ok_or_else(|| AppError::validation("Unknown Emoji difficulty."))?;
    ...
    let entity = entities
        ...
        .ok_or_else(|| {
            AppError::Unavailable("No Emoji are configured for this difficulty.".to_owned())
        })?;
    ...
    find_by_date_mode(pool, date, &mode)
        .await?
