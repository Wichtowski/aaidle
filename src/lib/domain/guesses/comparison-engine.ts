...
export function compareSets(a: string[] | null, b: string[] | null): SetComparison {
  ...
  if (left.size === right.size && [...left].every((value) => right.has(value))) {
    return "correct";
  }
...
export function compareNumber(guess: number | null, answer: number | null): NumberComparison {
  if (guess == null || answer == null) {
    return guess == null && answer == null ? "correct" : "unknown";
  }
...
export function compareClassicModels(
  ...
  return {
    ...
    supportedLanguages: compareSets(
    ...
    visionTasks: compareSets(
      vision(guessed)?.visionTasks ?? null,
      vision(answer)?.visionTasks ?? null,
    ),
    architecture: compareSets(
      language(guessed)?.architecture ??
        vision(guessed)?.architecture ??
        nlp(guessed)?.architecture ??
        detection(guessed)?.architecture ??
        null,
      language(answer)?.architecture ??
        vision(answer)?.architecture ??
        nlp(answer)?.architecture ??
        detection(answer)?.architecture ??
        null,
    ),
    trainingDatasets: compareSets(
      vision(guessed)?.trainingDatasets ??
        nlp(guessed)?.trainingDatasets ??
        detection(guessed)?.trainingDatasets ??
        null,
      vision(answer)?.trainingDatasets ??
        nlp(answer)?.trainingDatasets ??
        detection(answer)?.trainingDatasets ??
        null,
    ),
    ...
    detectionTypes: compareSets(
      detection(guessed)?.detectionTypes ?? null,
      detection(answer)?.detectionTypes ?? null,
    ),
    realTimeCapable: compareNullableBoolean(
      detection(guessed)?.realTimeCapable ?? null,
      detection(answer)?.realTimeCapable ?? null,
    ),
    algorithmTypes: compareSets(
      classical(guessed)?.algorithmTypes ?? null,
      classical(answer)?.algorithmTypes ?? null,
    ),
    learningParadigms: compareSets(
      classical(guessed)?.learningParadigms ?? null,
      classical(answer)?.learningParadigms ?? null,
    ),
    objectives: compareSets(
      classical(guessed)?.objectives ?? null,
      classical(answer)?.objectives ?? null,
    ),
    featureTypes: compareSets(
      classical(guessed)?.featureTypes ?? null,
      classical(answer)?.featureTypes ?? null,
    ),
    ...
    operationTypes: compareSets(
      filters(guessed)?.operationTypes ?? null,
      filters(answer)?.operationTypes ?? null,
    ),
    kernelBased: compareNullableBoolean(
      filters(guessed)?.kernelBased ?? null,
      filters(answer)?.kernelBased ?? null,
    ),
    kernelSizes: compareSets(
      filters(guessed)?.kernelSizes ?? null,
      filters(answer)?.kernelSizes ?? null,
    ),
    linearity: compareScalar(
      filters(guessed)?.linearity ?? null,
      filters(answer)?.linearity ?? null,
    ),
    requiresTraining: compareNullableBoolean(
      filters(guessed)?.requiresTraining ?? null,
      filters(answer)?.requiresTraining ?? null,
    ),
    outputTypes: compareSets(
      filters(guessed)?.outputTypes ?? null,
      filters(answer)?.outputTypes ?? null,
    ),
  };
}
