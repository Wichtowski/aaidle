  ...
  return (
    <section className="completed-model-space" aria-labelledby="completed-model-space-title">
      ...
      >
        ...
        {pathPoints.length > 1 && (
          <polyline
            className="completed-model-space__path"
            points={pathPoints
              .map((point) => `${point.projected.x},${point.projected.y}`)
              .join(" ")}
          />
        )}
        {pathPoints.at(-1) && answerPoint && (
          <line
            className="completed-model-space__destination"
            x1={pathPoints.at(-1)?.projected.x}
            x2={answerPoint.projected.x}
            y1={pathPoints.at(-1)?.projected.y}
        ...
        {points
          ...
          .map((point) => (
            <g
              className={`completed-model-space__point completed-model-space__point--${point.kind}`}
              key={`${point.kind}:${point.model.id}`}
              transform={`translate(${point.projected.x} ${point.projected.y})`}
            >
              <title>
                {point.kind === "answer"
                  ? `Winning answer: ${point.model.name}`
                  : `Guess ${point.attemptNumber}: ${point.model.name}`}
              </title>
        ...
        <text className="completed-model-space__axis-label" x="658" y="474">
          {axes[0].label}
        </text>
        <text
          className="completed-model-space__axis-label"
        ...
        <text className="completed-model-space__axis-label" x="590" y="33">
          {axes[2].label}
        </text>
      </svg>
      <div className="completed-model-space__legend">
        <span>
          <i data-kind="guess" /> Guess path
        </span>
        {answer && (
          <span>
            <i data-kind="answer" /> Answer · {answer.model.name}
          </span>
        )}
        <span>
          <i data-kind="reference" /> {references.length} candidate models
        </span>
      </div>
    </section>
  );
